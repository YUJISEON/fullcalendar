	...
	const calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        selectAllow: function(selectInfo) {
            ...
            for (let i = 0; i < selectedDates.length; i++) {
                if (unavailableDates.includes(selectedDates[i]))  return false;
                else if (peakSeasonDates.includes(selectedDates[i]))  return false;
                else if (fullyBookedDates.includes(selectedDates[i])) return false;
            }
            
            return true;
        },
        select: function(info) {           
            var selectedStartDate = info.startStr;
            var selectedEndDate = info.endStr;
            var selectedDates = getDatesBetween(selectedStartDate, selectedEndDate);
